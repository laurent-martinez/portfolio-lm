...
export default function Projects({}: Props) {
  ...
  return (
    <div className='h-screen flex flex-col mx-0 sm:mx-auto items-center justify-evenly relative overflow-hidden text-left md:flex-row max-w-full z-0'>
             <h3 className='absolute top-14 sm:top-10 uppercase tracking-[20px] text-gray-500 text-2xl'>Projects</h3>
             <div className='relative overflow-x-scroll overflow-y-hidden scrollbar scrollbar-track-gray-400/20 scrollbar-thumb-[#F7AB0A]/80 w-full flex snap-x snap-mandatory z-20'>
                { datas.map((project,index) =>(
                    <div key={index} className='w-screen flex-shrink-0 snap-center flex flex-col space-y-5 mb-10 items-center justify-center py-10 px-20 '>
                        ...
                        <Image src="/LesPetitsPlats-mobile.png" alt='photo mobile' width={170} height={360} className='w-[85px] h-[180px]  sm:hidden mt-0 pt-0'/>
                        <Image src="/portfolio-tablet.png" alt='photo tablet' width={387} height={408} className='hidden sm:flex lg:hidden w-[258px] h-[272px] mt-0 pt-0'/>
                        <div className=' px-0 md:px-10 max-w-6xl flex flex-col items-center'>
                            ...
                            <p className='text-lg text-center line-clamp-6 lg:line-clamp-none md:text-left font-roboto mb-4'>{project.text}</p>
                            <ul>
